...
- **احفظ العمل أوّلاً بأوّل (قاعدة دائمة لكل المهام):** بعد كل جزء مكتمل ومستقر اعمل
  **commit مرحلي** فوراً وارفعه — لا تُجمّع مهمة كبيرة في commit واحد بالنهاية، ولا تترك
  تغييرات كبيرة غير محفوظة/مرفوعة. قبل أي تعديل كبير أو محفوف بالمخاطر تأكّد أن آخر نسخة
  مستقرة محفوظة ويمكن الرجوع إليها. الهدف: إن توقّفت الجلسة أو انقطع الاتصال أو انتهى الحدّ،
  يستطيع مطوّر آخر (أو Codex) الإكمال مباشرةً من آخر commit بلا فقدان شيء. رسائل الـcommit
  واضحة تُبيّن ما أُنجز. **ممنوع** force-push أو حذف تاريخ Git أو تعديل commits سابقة إلا
  لضرورة واضحة.
...
9. **ميزة الأقسام (التصنيف) أُزيلت نهائياً من التطبيق — قرار دائم، لا تُعِد إضافتها.**
   كل واجهة المستخدم والمسارات وإدارتها من لوحة التحكم ومفتاح التفعيل حُذفت بالكامل
   (لا `/categories`، لا `/admin/categories`، لا `/admin/classifier`، لا `categoriesEnabled()`
   ولا `cats_on`). جداول `categories`/`sub_categories` وأعمدة `ads.category_id`/
   `ads.subcategory_id`/`ads.cat_reviewed` بقيت في قاعدة البيانات كما هي **دون أي تعديل**
   — بيانات خاملة فقط، ليست ميزة ظاهرة للمستخدم. التصنيف الآلي التلقائي
   (`classifyAdText()` في `src/lib/classifier.ts`) لا يزال يُسند كل إعلان جديد داخلياً
   لقسم «عروض أخرى» الاحتياطي عبر `getFallbackCategoryId()` — هذا حبر داخلي بحت
   للتوافق مع مخطط القاعدة، وليس ميزة تُعرض لأحد. لا يوجد مفتاح في لوحة الإدارة لإعادة
   تفعيل الأقسام — أي طلب لإعادتها مستقبلاً يحتاج عملاً جديداً من الصفر، لا مجرّد تبديل إعداد.
10. **عند أي بلاغ "شاشة خطأ" من عضو، راجع `/admin/errors` أولاً** بدل التخمين — كل خطأ
    تصيير يُسجَّل تلقائياً هناك (رسالة + رابط الصفحة + العضو). لعمليات حسّاسة (حفظ ملف
    مرفوع، إلخ) غلّفها بـ `try/catch` + `logClientError()` (`src/lib/error-log.ts`) بدل
    تركها تنفجر لشاشة الخطأ العامة.
