...
import { readLocal, writeLocal, persistLocal, type PreviewStorageMode } from '../lib/preview';
import './seller.css';
import './category-details.css';

export function FieldSettingsPreview({ storageMode = 'local' }: {storageMode?: PreviewStorageMode} = {}) {
  ...
  async function save() {
    ...
    if (storageMode === 'server') {
      savingRef.current = true; setSaving(true); setMessage('جارٍ حفظ إعدادات الحقول…');
      try {
        await persistLocal(FIELD_SETTINGS_KEY, settings, storageMode);
        setMessage('حُفظت إعدادات الحقول لحسابك على خادم التجربة.');
      } catch (error) { setMessage(error instanceof Error ? error.message : 'تعذر تأكيد الحفظ. تعديلاتك ما زالت في الصفحة.'); }
  ...
  return <div className="container seller-page compact-fields" aria-busy={saving}><fieldset disabled={saving || !ready} style={{display:'contents'}}><div className="seller-page-head"><div><h1>إعدادات حقول الأقسام</h1><p>{storageMode === 'server' ? 'إعدادات حقول حسابك في التجربة — لا تغيّر إعدادات الموقع الفعلي.' : 'معاينة أدوات الإدارة — لا تتصل بقاعدة بيانات الموقع الحي.'}</p></div>{storageMode === 'local' && <Link className="button secondary" href="/classification/">تصنيف الإعلانات</Link>}<Link className="button secondary" href="/ads/new/">تجربة إضافة إعلان</Link></div><div className="seller-demo-note">{storageMode === 'server' ? 'هذه الإعدادات خاصة بحسابك في بيئة التجربة، وليست إعدادات إدارة عامة. اضغط حفظ لتطبيقها على إعلاناتك التجريبية.' : 'يمكن لأي زائر تجربة هذه الصفحة. الإعدادات محلية له وحده، وليست لوحة إدارة فعلية أو صلاحيات على الموقع.'} إخفاء أو أرشفة حقل لا يحذف الإعلانات المحفوظة.</div><section className="seller-form-panel"><div className="seller-fields">
    ...
    <fieldset className="category-field-group"><legend>إضافة حقل لهذا الفرع فقط</legend><div className="category-field-grid"><label className="seller-field">اسم الحقل الجديد<input value={label} maxLength={80} onChange={e => setLabel(e.target.value)} /></label><label className="seller-field">نوع الحقل<select aria-label="نوع الحقل" value={type} onChange={e => setType(e.target.value as Field['type'])}><option value="text">نص</option><option value="number">رقم</option><option value="select">اختيار واحد (قائمة منسدلة)</option><option value="multi">اختيار متعدد (مربعات تحديد)</option><option value="date">تاريخ</option></select></label>{['select','multi'].includes(type) && <div className="choice-editor category-wide"><strong>خيارات الحقل</strong><small>كل خيار في سطر مستقل. الخيارات المكررة أو الفارغة لا تُضاف.</small>{options.map((option,index) => <div className="choice-row" key={index}><input aria-label={`الخيار ${index+1}`} value={option} maxLength={80} onChange={e => setOptions(options.map((item,i)=>i===index?e.target.value:item))} /><button type="button" aria-label={`حذف الخيار ${index+1}`} onClick={()=>setOptions(options.filter((_,i)=>i!==index))}>حذف</button></div>)}<button className="button secondary" type="button" disabled={options.length>=50} onClick={()=>setOptions([...options,''])}>إضافة خيار</button></div>}<div className="category-admin-controls category-wide"><label><input type="checkbox" checked={required} onChange={e=>setRequired(e.target.checked)} />الحقل الجديد إجباري</label><label><input type="checkbox" checked={visible} onChange={e=>setVisible(e.target.checked)} />إظهار الحقل الجديد</label></div><button className="button secondary" type="button" disabled={!label.trim() || (['select','multi'].includes(type) && !normalizeOptions(options).length) || (current.added?.length || 0)>=30} onClick={() => {const id=`custom${Date.now()}`;change({added:[...(current.added || []),{id,label:label.trim(),type,required,group:'تفاصيل إضافية',...(['select','multi'].includes(type)?{options:normalizeOptions(options)}:{})}],fields:{...current.fields,[id]:{hidden:!visible}}});setLabel('');setOptions(['','']);setRequired(false);setVisible(true);}}>إضافة الحقل</button></div></fieldset>
    <div className="seller-inline"><button className="button primary" disabled={!ready || saving} onClick={save}>حفظ إعدادات الحقول</button><button className="button secondary" onClick={() => {const next={...settings};delete next[key];setSettings(next);setMessage('استعدت التعريف الافتراضي لهذا الفرع. اضغط حفظ للتطبيق.');}}>استعادة حقول هذا الفرع</button><span role="status">{message}</span></div>
