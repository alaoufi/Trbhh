...
type AdForm = { intent: Intent; category: string; subcategory: string; title: string; description: string; price: string; region?: string; city: string; condition: string; images: string[]; spec1: string; spec2: string; details: Details };
...
function normalizeForm(form: AdForm): AdForm {
  ...
  const region = typeof form.region === 'string' ? form.region : inferRegion(form.city);
  return { ...form, region, details, condition: profile?.condition ? form.condition : '', price: profile?.pricing === 'salary' ? '' : form.price };
}
...
function restoreDraftForm(draft: AdDraft, seedAds: SellerAd[]): AdForm {
  ...
  if (validTarget(assignment) && assignment.updatedAt && assignment.updatedAt>draft.savedAt &&
    getAds(seedAds).find(ad=>ad.id===draft.editingId)?.classificationRevision===assignment.fromRevision) {
    return normalizeForm(assign([{...draft.form,id:draft.editingId}],[draft.editingId],assignment)[0]);
  }
  return draft.form;
}

function getAds(seedAds: SellerAd[] = EMPTY_ADS): SellerAd[] {
  const stored = readLocal<unknown>(SELLER_ADS_KEY, null);
  if (!Array.isArray(stored)) return seedAds.map(ad => resolveAd(ad,'local',readAssignments()));
...
function AdPreview({ ad, demo = false }: { ad: AdForm; demo?: boolean }) {
  const specs = displayDetails(getProfile(ad.category, ad.subcategory), ad.details);
  return <article className="seller-ad-preview"><AdPicture ad={ad} className="seller-preview-cover" /><div className="seller-preview-body"><div className="seller-inline"><span className="seller-pill gold">{ad.category === 'وظائف' ? (ad.intent === 'wanted' ? 'باحث عن عمل' : 'فرصة عمل') : ad.intent === 'wanted' ? 'مطلوب' : 'معروض'}</span><span className="seller-muted">{ad.category} · {ad.subcategory}</span></div><h2>{ad.title || 'عنوان إعلانك'}</h2><strong className="seller-preview-price">{priceText(ad)}</strong><p className="seller-inline seller-muted"><MapPin size={16} />{ad.city || 'المدينة'}{getProfile(ad.category, ad.subcategory)?.condition && ad.condition && <> · {ad.condition}</>}</p><p className="seller-description">{ad.description}</p>{specs.length > 0 && <dl className="seller-specs">{specs.map(([label, value], index) => <div key={`${label}-${index}`}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}{ad.images.length > 1 && <div className="seller-preview-thumbs">{ad.images.slice(1).map((src, index) => <img key={index} src={src} alt={`صورة إضافية ${index + 2}`} />)}</div>}{demo && <div className="seller-preview-owner"><span className="seller-avatar small">د</span><div><strong>متجرك التجريبي</strong><span className="seller-muted">سيظهر الإعلان هنا داخل المعاينة</span></div><ShieldCheck size={21} /></div>}</div></article>;
}
...
export function NewAdPage({ seedAds = EMPTY_ADS, demo = false, storageMode = 'local' }: LocalSellerProps = {}) {
  ...
  useEffect(() => {
    ...
    if (requestedEdit) {
      const ad = getAds(seedAds).find(item => item.id === requestedEdit);
      if (draft?.editingId === requestedEdit) {
        setForm(restoreDraftForm(draft, seedAds)); setEditingId(requestedEdit); setRestored(true);
      }
    ...
    } else if (draft) { setForm(restoreDraftForm(draft, seedAds)); setEditingId(draft.editingId || null); setRestored(true); }
  ...
  function validate(targetStep: number): Errors {
    ...
    if (targetStep === 1) {
      ...
      if (!citiesForRegion(form.region || '').length) result.region = 'اختر المنطقة.';
      if (!citiesForRegion(form.region || '').includes(form.city)) result.city = 'اختر مدينة تابعة للمنطقة المحددة.';
...
export function SellerDashboard({ seedAds = EMPTY_ADS, demo = false, storageMode = 'local' }: LocalSellerProps = {}) {
  ...
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => { const sync = () => { setAds(getAds(seedAds)); setDraft(getDraft()); }; sync(); setHydrated(true); window.addEventListener('storage', sync); window.addEventListener('trbhh-seller-updated', sync); return () => { window.removeEventListener('storage', sync); window.removeEventListener('trbhh-seller-updated', sync); }; }, []);
