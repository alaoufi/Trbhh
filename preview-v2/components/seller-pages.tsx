...
export function NewAdPage() {
  ...
  useEffect(() => {
    ...
    const snapshot: AdDraft = { version: 1, form, step, editingId, savedAt: Date.now() };
  ...
  }, [form, step, editingId, hydrated, complete, hasContent]);
  ...
  function goTo(next: number) {
    setErrors({}); setStep(next); setSaveStatus('saving');
    window.setTimeout(() => stepTitle.current?.focus({ preventScroll: true }), 0);
  }
  function next() {
  ...
  function publish() {
    for (const target of [0, 1]) {
      ...
      if (Object.keys(found).length) { setErrors(found); setStep(target); return; }
    }
    const ads = getAds();
    ...
    if (!Array.isArray(persisted) || JSON.stringify(persisted.find(ad => ad.id === savedAd.id)) !== JSON.stringify(savedAd)) {
      const retryDraft: AdDraft = { version: 1, form, step, editingId, savedAt: Date.now() };
  ...
  function resetDraft() {
    latestDraft.current = null;
    writeLocal(AD_DRAFT_KEY, null); setForm(emptyForm); setStep(0); setEditingId(null); setRestored(false); setErrors({}); setSaveStatus('idle'); setDiscardOpen(false); setUploadError('');
  ...
  return <div className="container seller-page seller-create-page">
    ...
    <div className="seller-page-head"><div><span className="seller-eyebrow">ابدأ حكاية بيع جديدة</span><h1>{editingId ? 'عدّل إعلانك' : 'إعلانك يبدأ من هنا'}</h1><p>تفاصيل واضحة وصور جيدة تقرّبك من المشتري المناسب.</p></div><Link href="/seller/" className="seller-text-link">لوحة البائع<ArrowLeft size={18} /></Link></div>
    ...
    <div className="seller-form-layout"><div className="seller-form-main">
      <ol className="seller-steps" aria-label="خطوات إضافة الإعلان">{steps.map((label, index) => <li key={label} className={index === step ? 'current' : index < step ? 'done' : ''} aria-current={index === step ? 'step' : undefined}><span>{index < step ? <Check size={16} /> : index + 1}</span><strong>{label}</strong></li>)}</ol>
