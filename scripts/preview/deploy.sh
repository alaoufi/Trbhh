...
set -euo pipefail
umask 077
preview_root=/root/trbhh-categories-preview-login
...
cd "$preview_root"
[[ -f .preview-only && "$(cat .preview-only)" == 'trbhh-categories-preview-login' ]] || exit 1
...
if [[ ! -f .env ]]; then
  ...
  docker compose -p trbhh-categories-preview-login -f compose.yml exec -T preview-db \
...
docker compose -p trbhh-categories-preview-login -f compose.yml config --quiet
docker compose -p trbhh-categories-preview-login -f compose.yml up -d --build --wait --wait-timeout 360

for attempt in $(seq 1 36); do
  preview_url=$(docker compose -p trbhh-categories-preview-login -f compose.yml logs --no-color preview-tunnel 2>&1 \
