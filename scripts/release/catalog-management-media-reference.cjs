'use strict';
// Bind catalog price, visibility and removal controls to the exact backup that deployed
// and verified the prior onboarding correction. Media stays in the immutable chain.
const fs = require('node:fs');
const path = require('node:path');
const {createHash} = require('node:crypto');
const {isDeepStrictEqual} = require('node:util');
const {spawnSync} = require('node:child_process');
const onboardingAudit = require('./onboarding-audit-compact-media-reference.cjs');
...
const CHECKPOINT_ID='35656382084';
const BASELINE='1b700dbdcb7d6a2c6fba8ea2cc03cb50aeb20b7d';
const CURRENT='6dc58319b9d570c32f4c983491e8950689292b31';
const hash=value=>createHash('sha256').update(value).digest('hex');
function check(value){if(!value)throw Error('catalog_management_media_reference_rejected');}
...
function inspectCheckpoint(checkpoint,base=BASE){
 ...
 check(onboardingAudit.verify(checkpoint,base).ok===true);
 ...
 run('verify-runtime.cjs',[path.join(checkpoint,'container-before.json'),path.join(checkpoint,'container-after.json'),'onboarding_audit_compact']);
 ...
 const inherited=json(checkpoint,'ONBOARDING_AUDIT_COMPACT_MEDIA_REFERENCE.json'),mediaAfter={};
 ...
 for(const name of ['database.sql.gz','code.tar.gz','image.tar.gz','ONBOARDING_AUDIT_COMPACT_MEDIA_REFERENCE.json','REUSED_MEDIA_SOURCE','container-before.json','supplier-before.json','full-before.json',...inherited.media.flatMap(item=>[`${item.label}.path`,`${item.label}-before.json`])])check(Object.hasOwn(archives,name));
