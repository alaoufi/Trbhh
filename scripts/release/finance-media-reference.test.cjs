...
function verifiedFinanceParent(f){
  ...
  const manifest=f.manifest,full={format:'trbhh-database-full-proof-v1',capturedAt:'before',tables:{users:{count:2,engine:'InnoDB',columns:['id'],primaryKeyColumns:['id'],schemaSha256:hash('schema'),rowHashes:[hash('user-a'),hash('user-b')]},ads:{count:2,engine:'InnoDB',columns:['id'],primaryKeyColumns:['id'],schemaSha256:hash('schema'),rowHashes:[hash('ad-a'),hash('ad-b')]}}},restored=structuredClone(full);
  restored.capturedAt='after';for(const table of Object.values(restored.tables))table.rowHashes.reverse();
  const files={'commit.txt':'353ae4c023e1183452b38be3f6fe8b9284d047c8\n','candidate.txt':'19e4c6f72c35f4adf2cb96600da094ef46788e2b\n','container-before.json':'[]','legacy.path':'/app/legacy\n','storage.path':'/app/storage\n','legacy-before.json':manifest,'storage-before.json':manifest,'legacy.tar.gz':'legacy-media','storage.tar.gz':'storage-media','code.tar.gz':'code','database.sql.gz':'database','image.tar.gz':'image','full-before.json':JSON.stringify(full),'full-restored.json':JSON.stringify(restored)};
