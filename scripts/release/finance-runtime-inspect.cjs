...
async function main(){
 ...
 try{
  ...
  const profiles=tables.includes('supplier_integration_profiles')?await db.$queryRawUnsafe('SELECT supplier_id,provider,environment,active,maintenance,products_sync_enabled,orders_sync_enabled,last_error FROM supplier_integration_profiles ORDER BY supplier_id'):[];
