'use strict';
// Bind this legacy connected-supplier email correction release to the exact backup that deployed
...
const onboardingTemplate = require('./onboarding-template-media-reference.cjs');
...
const CHECKPOINT_ID='35649868888';
const BASELINE='408fb10d18ddad9f83259ffd8d006e441e6a6515';
const CURRENT='97563332511adda499f86ec87a0df54aaf4551b9';
const hash=value=>createHash('sha256').update(value).digest('hex');
function check(value){if(!value)throw Error('onboarding_legacy_email_media_reference_rejected');}
...
function inspectCheckpoint(checkpoint,base=BASE){
 ...
 check(onboardingTemplate.verify(checkpoint,base).ok===true);
 ...
 run('verify-runtime.cjs',[path.join(checkpoint,'container-before.json'),path.join(checkpoint,'container-after.json'),'onboarding_template']);
 ...
 const inherited=json(checkpoint,'ONBOARDING_TEMPLATE_MEDIA_REFERENCE.json'),mediaAfter={};
 ...
 for(const name of ['database.sql.gz','code.tar.gz','image.tar.gz','ONBOARDING_TEMPLATE_MEDIA_REFERENCE.json','REUSED_MEDIA_SOURCE','container-before.json','supplier-before.json','full-before.json',...inherited.media.flatMap(item=>[`${item.label}.path`,`${item.label}-before.json`])])check(Object.hasOwn(archives,name));
