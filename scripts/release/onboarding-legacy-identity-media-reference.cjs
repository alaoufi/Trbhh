'use strict';
// Bind the legacy connected-supplier identity correction to the exact backup that deployed
// and verified the prior email correction. Media stays in the immutable chain.
const fs = require('node:fs');
const path = require('node:path');
const {createHash} = require('node:crypto');
const {isDeepStrictEqual} = require('node:util');
const {spawnSync} = require('node:child_process');
const legacyEmail = require('./onboarding-legacy-email-media-reference.cjs');
...
const CHECKPOINT_ID='35651818861';
const BASELINE='97563332511adda499f86ec87a0df54aaf4551b9';
const CURRENT='706f1da6999def43dab8d329ba3b576f3e764a87';
const hash=value=>createHash('sha256').update(value).digest('hex');
function check(value){if(!value)throw Error('onboarding_legacy_identity_media_reference_rejected');}
...
function inspectCheckpoint(checkpoint,base=BASE){
 ...
 check(legacyEmail.verify(checkpoint,base).ok===true);
 ...
 run('verify-runtime.cjs',[path.join(checkpoint,'container-before.json'),path.join(checkpoint,'container-after.json'),'onboarding_legacy_email']);
 check(supplier.verify(json(checkpoint,'supplier-before.json'),json(checkpoint,'supplier-after.json')).ok===true);
 const databaseResult=JSON.parse(run('database-proof.cjs',['verify-known-migration',path.join(checkpoint,'before.json'),path.join(checkpoint,'after.json'),'msg_topup_ok_v1']));check(databaseResult.ok===true&&databaseResult.sameDatabase===true&&databaseResult.failures.length===0);
 const inherited=json(checkpoint,'ONBOARDING_LEGACY_EMAIL_MEDIA_REFERENCE.json'),mediaAfter={};
 ...
 for(const name of ['database.sql.gz','code.tar.gz','image.tar.gz','ONBOARDING_LEGACY_EMAIL_MEDIA_REFERENCE.json','REUSED_MEDIA_SOURCE','container-before.json','supplier-before.json','full-before.json',...inherited.media.flatMap(item=>[`${item.label}.path`,`${item.label}-before.json`])])check(Object.hasOwn(archives,name));
 ...
 return {format:'trbhh-onboarding-legacy-identity-media-reference-v1',checkpointId:CHECKPOINT_ID,checkpointPath:checkpoint,checkpointBaseline:BASELINE,checkpointDeployment:CURRENT,checkpointRuntimeImage:after.image,markers,checkpointSha256Sums:hash(sumsRaw),archives,proofs:Object.fromEntries(proofNames.map(name=>[name,hashFile(path.join(checkpoint,name))])),mediaAfter,inheritedReferenceSha256:hashFile(path.join(checkpoint,'ONBOARDING_LEGACY_EMAIL_MEDIA_REFERENCE.json')),media:inherited.media};
}
function prepare(checkpoint,child,base=BASE){childDirectory(child,base);const reference=inspectCheckpoint(checkpoint,base);check(!fs.existsSync(path.join(child,'ONBOARDING_LEGACY_IDENTITY_MEDIA_REFERENCE.json'))&&!fs.existsSync(path.join(child,'REUSED_MEDIA_SOURCE')));selection.sameRuntime(selection.container(readSmall(path.join(checkpoint,'container-after.json'))),selection.container(readSmall(path.join(child,'container-before.json'))));for(const item of reference.media)for(const suffix of ['.path','-before.json'])fs.writeFileSync(path.join(child,item.label+suffix),readSmall(path.join(checkpoint,item.label+suffix)),{flag:'wx',mode:0o600});fs.writeFileSync(path.join(child,'ONBOARDING_LEGACY_IDENTITY_MEDIA_REFERENCE.json'),`${JSON.stringify(reference)}\n`,{flag:'wx',mode:0o600});fs.writeFileSync(path.join(child,'REUSED_MEDIA_SOURCE'),`${CHECKPOINT_ID}\n`,{flag:'wx',mode:0o600});return {ok:true,referencedMedia:reference.media.length};}
function verify(child,base=BASE){childDirectory(child,base);check(readSmall(path.join(child,'REUSED_MEDIA_SOURCE')).toString().trim()===CHECKPOINT_ID);const saved=json(child,'ONBOARDING_LEGACY_IDENTITY_MEDIA_REFERENCE.json'),current=inspectCheckpoint(path.join(base,`audit-${CHECKPOINT_ID}`),base);check(isDeepStrictEqual(saved,current));selection.sameRuntime(selection.container(readSmall(path.join(current.checkpointPath,'container-after.json'))),selection.container(readSmall(path.join(child,'container-before.json'))));for(const item of current.media){check(hashFile(path.join(child,`${item.label}.path`))===hashFile(path.join(current.checkpointPath,`${item.label}.path`)));check(hashFile(path.join(child,`${item.label}-before.json`))===hashFile(path.join(current.checkpointPath,`${item.label}-before.json`)));check(!fs.existsSync(path.join(child,`${item.label}.tar.gz`))&&!fs.existsSync(path.join(child,`${item.label}-extracted`)));}return {ok:true,referencedMedia:current.media.length};}
if(require.main===module){try{const [mode,first,second,...extra]=process.argv.slice(2);check(extra.length===0);const result=mode==='prepare'&&first&&second?prepare(first,second):mode==='verify'&&first&&!second?verify(first):null;check(result);process.stdout.write(`${JSON.stringify(result)}\n`);}catch{process.stderr.write('Onboarding legacy identity checkpoint review failed; retain all backups. Private values withheld.\n');process.exitCode=1;}}
