'use strict';
// Bind this supplier onboarding and owner-link release to the exact backup that deployed
...
const bannerSeparation = require('./banner-separation-media-reference.cjs');
...
const CHECKPOINT_ID='35642667045';
const BASELINE='7def385fc2a161f829486544815b6f3a58e327e7';
const CURRENT='c1fbdf0e18f263c1881b4092ae8ddad74cc87b98';
const hash=value=>createHash('sha256').update(value).digest('hex');
function check(value){if(!value)throw Error('onboarding_link_media_reference_rejected');}
...
function inspectCheckpoint(checkpoint,base=BASE){
 ...
 check(bannerSeparation.verify(checkpoint,base).ok===true);
 ...
 run('verify-runtime.cjs',[path.join(checkpoint,'container-before.json'),path.join(checkpoint,'container-after.json'),'banner_separation']);
 ...
 const inherited=json(checkpoint,'BANNER_SEPARATION_MEDIA_REFERENCE.json'),mediaAfter={};
 ...
 for(const name of ['database.sql.gz','code.tar.gz','image.tar.gz','BANNER_SEPARATION_MEDIA_REFERENCE.json','REUSED_MEDIA_SOURCE','container-before.json','supplier-before.json','full-before.json',...inherited.media.flatMap(item=>[`${item.label}.path`,`${item.label}-before.json`])])check(Object.hasOwn(archives,name));
 ...
 return {format:'trbhh-onboarding-link-media-reference-v1',checkpointId:CHECKPOINT_ID,checkpointPath:checkpoint,checkpointBaseline:BASELINE,checkpointDeployment:CURRENT,checkpointRuntimeImage:after.image,markers,checkpointSha256Sums:hash(sumsRaw),archives,proofs:Object.fromEntries(proofNames.map(name=>[name,hashFile(path.join(checkpoint,name))])),mediaAfter,inheritedReferenceSha256:hashFile(path.join(checkpoint,'BANNER_SEPARATION_MEDIA_REFERENCE.json')),media:inherited.media};
}
function prepare(checkpoint,child,base=BASE){childDirectory(child,base);const reference=inspectCheckpoint(checkpoint,base);check(!fs.existsSync(path.join(child,'ONBOARDING_LINK_MEDIA_REFERENCE.json'))&&!fs.existsSync(path.join(child,'REUSED_MEDIA_SOURCE')));selection.sameRuntime(selection.container(readSmall(path.join(checkpoint,'container-after.json'))),selection.container(readSmall(path.join(child,'container-before.json'))));for(const item of reference.media)for(const suffix of ['.path','-before.json'])fs.writeFileSync(path.join(child,item.label+suffix),readSmall(path.join(checkpoint,item.label+suffix)),{flag:'wx',mode:0o600});fs.writeFileSync(path.join(child,'ONBOARDING_LINK_MEDIA_REFERENCE.json'),`${JSON.stringify(reference)}\n`,{flag:'wx',mode:0o600});fs.writeFileSync(path.join(child,'REUSED_MEDIA_SOURCE'),`${CHECKPOINT_ID}\n`,{flag:'wx',mode:0o600});return {ok:true,referencedMedia:reference.media.length};}
function verify(child,base=BASE){childDirectory(child,base);check(readSmall(path.join(child,'REUSED_MEDIA_SOURCE')).toString().trim()===CHECKPOINT_ID);const saved=json(child,'ONBOARDING_LINK_MEDIA_REFERENCE.json'),current=inspectCheckpoint(path.join(base,`audit-${CHECKPOINT_ID}`),base);check(isDeepStrictEqual(saved,current));selection.sameRuntime(selection.container(readSmall(path.join(current.checkpointPath,'container-after.json'))),selection.container(readSmall(path.join(child,'container-before.json'))));for(const item of current.media){check(hashFile(path.join(child,`${item.label}.path`))===hashFile(path.join(current.checkpointPath,`${item.label}.path`)));check(hashFile(path.join(child,`${item.label}-before.json`))===hashFile(path.join(current.checkpointPath,`${item.label}-before.json`)));check(!fs.existsSync(path.join(child,`${item.label}.tar.gz`))&&!fs.existsSync(path.join(child,`${item.label}-extracted`)));}return {ok:true,referencedMedia:current.media.length};}
if(require.main===module){try{const [mode,first,second,...extra]=process.argv.slice(2);check(extra.length===0);const result=mode==='prepare'&&first&&second?prepare(first,second):mode==='verify'&&first&&!second?verify(first):null;check(result);process.stdout.write(`${JSON.stringify(result)}\n`);}catch{process.stderr.write('Onboarding link checkpoint review failed; retain all backups. Private values withheld.\n');process.exitCode=1;}}
