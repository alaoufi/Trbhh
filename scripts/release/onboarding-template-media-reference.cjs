'use strict';
// Bind this supplier-template correction release to the exact backup that deployed
...
const onboardingLink = require('./onboarding-link-media-reference.cjs');
...
const CHECKPOINT_ID='35644711205';
const BASELINE='c1fbdf0e18f263c1881b4092ae8ddad74cc87b98';
const CURRENT='408fb10d18ddad9f83259ffd8d006e441e6a6515';
const hash=value=>createHash('sha256').update(value).digest('hex');
function check(value){if(!value)throw Error('onboarding_template_media_reference_rejected');}
...
function inspectCheckpoint(checkpoint,base=BASE){
 ...
 check(onboardingLink.verify(checkpoint,base).ok===true);
 ...
 run('verify-runtime.cjs',[path.join(checkpoint,'container-before.json'),path.join(checkpoint,'container-after.json'),'onboarding_link']);
 ...
 const inherited=json(checkpoint,'ONBOARDING_LINK_MEDIA_REFERENCE.json'),mediaAfter={};
 ...
 for(const name of ['database.sql.gz','code.tar.gz','image.tar.gz','ONBOARDING_LINK_MEDIA_REFERENCE.json','REUSED_MEDIA_SOURCE','container-before.json','supplier-before.json','full-before.json',...inherited.media.flatMap(item=>[`${item.label}.path`,`${item.label}-before.json`])])check(Object.hasOwn(archives,name));
