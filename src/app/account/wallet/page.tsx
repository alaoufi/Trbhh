...
export default async function WalletPage({ searchParams }: { searchParams: Promise<{ dup?: string; topup?: string; error?: string; price?: string; bal?: string; page?: string; pts?: string; dupr?: string; dupamt?: string; duprel?: string; duphash?: string; tab?: string; service?: string; q?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {sp.error === 'payfailed' && <div className="rounded-lg border-2 border-red-400 bg-red-50 p-3 text-sm font-bold text-red-900">تعذّر إتمام الدفع الإلكتروني أو أُلغيت العملية — لم يُخصم منك شيء. جرّب مجدداً{topupMethods.transfer ? ' أو استخدم التحويل البنكي.' : '.'}</div>}
      {sp.error === 'transferoff' && <div className="rounded-lg border-2 border-amber-400 bg-amber-50 p-3 text-sm font-bold text-amber-900">التحويل البنكي موقوف مؤقتاً من الإدارة. استخدم الدفع الإلكتروني عند توفره.</div>}
