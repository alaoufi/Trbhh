...
export async function saveTextsAction(formData: FormData) {
  ...
  if (sec === 'general') {
    ...
    await put(SETTING_DISCLAIMER_LONG, 'disclaimerLong');
    await put(SETTING_DISCLAIMER_SHORT, 'disclaimerShort');
    await put(SETTING_SITE_CR, 'siteCr');
    await put(SETTING_SITE_FAL, 'siteFal');
  } else if (sec === 'home') {
