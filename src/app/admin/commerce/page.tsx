...
function ProductForm({ product }: { product?: Product }) {
  return <AccessBoundary module={'products'} action={product ? 'edit' : 'create'}><form action={saveCommerceProduct} className="grid gap-2 rounded-xl border border-primary/15 p-3 sm:grid-cols-2">
...
export default async function CommerceAdminPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const session = await requireAdminPage('/admin/commerce');
  ...
  const [sp, settings, products, orders, notices, gateway] = await Promise.all([
    searchParams,
    keys.has('products:manage_settings') ? prisma.site_settings.findMany({ where: { k: { in: Object.keys(COMMERCE_DEFAULTS) } } }) : [],
    ...
    keys.has('orders:view') ? prisma.$queryRaw<Order[]>`SELECT id,member_id,total_minor,status,fulfillment_status FROM commerce_orders ORDER BY id DESC LIMIT 100` : [],
    keys.has('notifications:view') ? prisma.$queryRaw<Notice[]>`SELECT id,order_id,channel,status FROM commerce_notifications WHERE channel IN ('sms','whatsapp') ORDER BY id DESC LIMIT 100` : [],
    getCommerceGateway(),
  ]);
  ...
  return <div className="space-y-4">
    ...
    <section className="card-3d overflow-x-auto rounded-xl p-4"><h2 className="mb-2 font-bold">آخر 100 طلب</h2>
      <table className="w-full text-right text-sm"><thead><tr><th>الطلب</th><th>العضو</th><th>الإجمالي</th><th>الدفع</th><th>تنفيذ المورد خارج الموقع</th></tr></thead>
        <tbody>{orders.map(o => <tr key={o.id.toString()} className="border-t"><td className="py-2"><AccessPage href={`/admin/commerce/orders/${o.id}`}><Link href={`/admin/commerce/orders/${o.id}`} className="text-primary underline">{o.id.toString()}</Link></AccessPage></td><td>{o.member_id.toString()}</td><td>{formatSar(o.total_minor)} ر.س</td><td>{o.status}</td><td>{o.fulfillment_status}</td></tr>)}</tbody></table>
    </section>
    <section className="card-3d rounded-xl p-4"><h2 className="font-bold">تنبيهات الدفع</h2><p className="my-2 text-xs">الحالة sending أو unknown تحتاج مراجعة لدى مزود الرسائل؛ لا تُعاد تلقائيًا. زر الإرسال يتصل بالمزود الفعلي وقد تُحسب تكلفته.</p>
      {notices.map(n => <div key={n.id.toString()} className="flex flex-wrap items-center gap-3 border-t py-2 text-sm"><span>طلب #{n.order_id.toString()} · {n.channel} · {n.status}</span>
        {n.status === 'pending' && <AccessBoundary module={'notifications'} action={'create'}><form action={deliverCommerceNotification}><input type="hidden" name="id" value={n.id.toString()} /><label><input name="confirm" value="1" type="checkbox" required /> تأكيد إرسال هذه الرسالة</label> <button className={button}>إرسال</button></form></AccessBoundary>}
      </div>)}
    </section>
  </div>;
}
