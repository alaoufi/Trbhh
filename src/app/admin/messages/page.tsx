...
export default async function AdminMessages({ searchParams }: { searchParams: Promise<Params> }) {
  const session = await requireAdminPage('/admin/messages');
  ...
  if (memberId > 0 && memberId !== adminId) {
    ...
    return <div className="mx-auto max-w-2xl space-y-3">
      <AccessPage href={`/admin/messages?tab=${tab}`}><Link href={`/admin/messages?tab=${tab}`} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary"><ArrowRight className="h-4 w-4" /> {tab === 'archived' ? 'أرشيف المراسلات' : 'مراسلات تحتاج إجراء'}</Link></AccessPage>
      <div className="flex flex-wrap items-center justify-between gap-2"><div><h1 className="flex items-center gap-2 text-lg font-bold text-primary"><MessagesSquare className="h-5 w-5" /> {thread.bName}</h1><p className="text-xs text-muted-foreground">{tab === 'archived' ? 'هذه المحادثة مؤرشفة ولا تظهر في التنبيهات.' : 'اختر رداً أو أرشف المحادثة بعد إنهاء الإجراء.'}</p></div>
        <div className="flex flex-wrap gap-2">
          ...
          {tab === 'open' && canEdit && <AccessBoundary module={'messages'} action={'edit'}><form action={archiveAdminMessageThreadAction}><input type="hidden" name="memberId" value={memberId} /><ConfirmSubmit msg="أرشفة هذه المحادثة؟ ستخرج من التنبيهات ويمكن استعادتها لاحقاً من الأرشيف." title="أرشفة المحادثة" className="inline-flex items-center gap-1 rounded-lg border border-amber-500 px-3 py-2 text-sm font-bold text-amber-800 hover:bg-amber-50"><Archive className="h-4 w-4" /> أرشفة المحادثة</ConfirmSubmit></form></AccessBoundary>}
          {tab === 'archived' && canEdit && <AccessBoundary module={'messages'} action={'edit'}><form action={restoreAdminMessageThreadAction}><input type="hidden" name="memberId" value={memberId} /><button type="submit" className="inline-flex items-center gap-1 rounded-lg border border-primary px-3 py-2 text-sm font-bold text-primary hover:bg-primary/5"><RotateCcw className="h-4 w-4" /> استعادة للمراسلات المفتوحة</button></form></AccessBoundary>}
          {tab === 'archived' && canDelete && <AccessBoundary module={'messages'} action={'delete'}><form action={deleteArchivedAdminMessageThreadAction}><input type="hidden" name="memberId" value={memberId} /><ConfirmSubmit msg="حذف المحادثة المؤرشفة وجميع رسائلها نهائياً؟ لا يمكن التراجع أو الاستعادة بعد الحذف." title="حذف المحادثة نهائياً" className="inline-flex items-center gap-1 rounded-lg bg-red-600 px-3 py-2 text-sm font-bold text-white hover:bg-red-700"><Trash2 className="h-4 w-4" /> حذف المحادثة نهائياً</ConfirmSubmit></form></AccessBoundary>}
        </div>
      </div>
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2 rounded-xl bg-secondary/70 p-2"><AccessPage href="/admin/messages?tab=open"><Link href="/admin/messages?tab=open" className={`rounded-lg px-3 py-2 text-sm font-bold ${tab === 'open' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-background'}`}>تحتاج إجراء</Link></AccessPage><AccessPage href="/admin/messages?tab=archived"><Link href="/admin/messages?tab=archived" className={`rounded-lg px-3 py-2 text-sm font-bold ${tab === 'archived' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-background'}`}>الأرشيف</Link></AccessPage></div>
