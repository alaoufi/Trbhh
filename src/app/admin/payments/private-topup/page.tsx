...
export default async function PrivateAlrajhiTopupPage({ searchParams }: { searchParams: Promise<{ state?: string; reason?: string; min?: string; max?: string }> }) {
  await requireAdminPage('/admin/payments/private-topup');
  ...
  return <main className="mx-auto max-w-xl space-y-4" dir="rtl">
    ...
    <AccessBoundary module={'payments'} action={'manage_settings'}><PrivateAlrajhiTopupForm action={startAlrajhiPrivateTopupAction} min={cfg.min} max={cfg.max} ready={ready} /></AccessBoundary>
  </main>;
}
