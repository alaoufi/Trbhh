...
export default async function AlrajhiSandboxPage({ searchParams }: { searchParams: Promise<{ state?: string; result?: string; reason?: string }> }) {
  await requireAdminPage('/admin/payments/sandbox');
  ...
  return <div className="mx-auto max-w-xl space-y-4" dir="rtl">
    ...
    <AccessBoundary module={'payments'} action={'manage_settings'}><form action={startAlrajhiSandboxAction} className="space-y-3 rounded-2xl border bg-card p-4">
