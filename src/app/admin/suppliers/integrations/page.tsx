...
import {SALLA_OAUTH_SCOPE_VERSION} from '@/lib/suppliers/salla-oauth';
...
type Connection={id:bigint;supplier_id:bigint;external_store_id:string;status:string;oauth_scope_version:number};
...
export default async function Integrations({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}) {
 ...
 prisma.$queryRaw<Connection[]>`SELECT c.id,c.supplier_id,c.external_store_id,c.status,c.oauth_scope_version FROM supplier_connections c JOIN (SELECT id FROM commerce_suppliers ORDER BY id DESC LIMIT 25 OFFSET ${supplierOffset}) s ON s.id=c.supplier_id ORDER BY c.id DESC LIMIT 50 OFFSET ${connectionOffset}`,
 ...
 {s.provider==='salla'&&!connections.some(c=>c.supplier_id===s.id&&c.oauth_scope_version===SALLA_OAUTH_SCOPE_VERSION)&&<SupplierOwnerInviteButton supplierId={String(s.id)} supplierName={s.name}/>}
 {s.provider==='salla'&&!connections.some(c=>c.supplier_id===s.id)&&<details><summary className="cursor-pointer text-sm underline">ربط مباشر من جلسة المشرف</summary><form action={connectSalla} className="mt-2"><input type="hidden" name="supplierId" value={String(s.id)}/><button className={button}>متابعة الربط في Salla</button></form></details>}
 {connections.filter(c=>c.supplier_id===s.id).map(c=>{const current=c.oauth_scope_version===SALLA_OAUTH_SCOPE_VERSION;return <div key={String(c.id)} className={`rounded border p-3 ${current?'border-emerald-200 bg-emerald-50':'border-amber-300 bg-amber-50'}`}><div className="flex flex-wrap items-center gap-2"><span className="font-semibold">المتجر {c.external_store_id} · {current?c.status:'إعادة التفويض مطلوبة'}</span>{current&&<form action={synchronize}><input type="hidden" name="id" value={String(c.id)}/><button className={button}>مزامنة الآن</button></form>}<form action={disconnectSalla}><input type="hidden" name="id" value={String(c.id)}/><button className="rounded border bg-white px-3 py-2">فصل الاتصال</button></form></div><p className="mt-2 text-xs leading-6">{current?'الاتصال يحمل جيل الصلاحيات الحالي. الرابط القديم يصبح غير صالح بعد اكتمال الربط أو إنشاء رابط أحدث.':'لن تستخدم تربح هذا الاتصال للمزامنة أو إنشاء الطلبات حتى يعيد صاحب المتجر التفويض ويوافق على صلاحية حفظ العميل.'}</p></div>;})}
 </article>)}</section>
