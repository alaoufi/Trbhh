...
export default async function AdminWords() {
  await requireAdminPage('/admin/words');
  ...
  return (
    <div className="space-y-4">
      ...
      <AccessBoundary module={'words'} action={'create'}><form action={addBannedWordAction} className="flex gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        {words.map((w) => (
          <div key={w.id} className="flex items-center gap-2 rounded-lg border border-primary/20 bg-card px-3 py-2 text-sm">
            ...
            <AccessBoundary module={'words'} action={'delete'}><form action={deleteBannedWordAction}>
      ...
      <div className="mt-6 space-y-3 border-t-2 border-primary/10 pt-5">
        ...
        <AccessBoundary module={'words'} action={'create'}><form action={addNameWordAction} className="flex gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {nameWords.map((w) => (
            <div key={w.id} className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50/60 px-3 py-2 text-sm">
              <span className="font-medium">{w.word}</span>
              <AccessBoundary module={'words'} action={'delete'}><form action={deleteNameWordAction}>
                <input type="hidden" name="id" value={w.id} />
