...
export default async function NewAuctionPage({ searchParams }: { searchParams: Promise<{ ad?: string; error?: string }> }) {
  ...
  const canAfford = cfg.fee <= 0 || balance >= cfg.fee;
  return (
    <div className="mx-auto max-w-lg space-y-4">
      ...
      <form action={createAuctionAction} className="card-3d rounded-2xl p-4">
        <fieldset disabled={!canAfford} className={`space-y-3 border-0 p-0 ${!canAfford ? 'opacity-40' : ''}`}>
