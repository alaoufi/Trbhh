...
export default async function CjStorePage({ searchParams }: { searchParams?: Promise<CjCatalogQuery> } = {}) {
  ...
  return <div className="mx-auto max-w-6xl min-w-0 space-y-5 px-3 py-5 sm:px-5" data-cj-trial="catalog">
    ...
    {catalog.items.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{catalog.items.map(item => item.source === 'cj' ? <CjProductCard key={item.key} product={item.product} /> : <AdCardMarketplace key={item.key} ad={item.ad} />)}</div> : <p className="rounded-2xl border bg-white p-8 text-center text-slate-500">لا توجد نتائج مؤهلة للعرض في هذا التبويب.</p>}
