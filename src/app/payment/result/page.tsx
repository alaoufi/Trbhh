...
export default async function PaymentResultPage({ searchParams }: { searchParams: Promise<{ t?: string }> }) {
  ...
  const title = success ? 'تم الدفع بنجاح' : failed ? 'لم تكتمل عملية الدفع' : 'جار التحقق من الدفع آلياً';
  const text = success ? 'أُضيف المبلغ إلى رصيدك تلقائياً بعد تأكيد البوابة.' : failed ? (topup.note || 'لم يُضف أي مبلغ إلى رصيدك. جرّب مجدداً أو استخدم التحويل البنكي.') : 'يجري التحقق آلياً مع بوابة البنك الآن؛ لا يلزم منك أو من الإدارة أي تأكيد. ستتحدث الصفحة تلقائياً.';
  ...
  return <section className={`mx-auto max-w-lg space-y-5 rounded-2xl border-2 p-6 text-center ${color}`}>{!success && !failed && <meta httpEquiv="refresh" content="5" />}<Icon className="mx-auto h-14 w-14" /><div><h1 className="text-2xl font-extrabold">{title}</h1><p className="mt-2 text-sm font-medium leading-6">{text}</p></div><dl className="grid grid-cols-2 gap-2 rounded-xl bg-white/80 p-4 text-right text-sm"><div><dt className="text-muted-foreground">رقم العملية</dt><dd className="font-extrabold">#{topup.id}</dd></div><div><dt className="text-muted-foreground">المبلغ</dt><dd className="font-extrabold">{topup.amount} ر.س</dd></div><div><dt className="text-muted-foreground">طريقة الدفع</dt><dd className="font-extrabold">{topup.method || 'بوابة إلكترونية'}</dd></div>{success && <div><dt className="text-muted-foreground">الرصيد الحالي</dt><dd className="font-extrabold">{balance} ر.س</dd></div>}</dl><div className="flex flex-wrap justify-center gap-2"><Link href="/account/wallet?tab=topups" className="btn-3d rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white">سجل شحن الرصيد</Link>{!success && <Link href="/account/wallet?tab=topups#topup" className="rounded-xl border-2 border-current px-5 py-3 text-sm font-bold">شحن رصيدي</Link>}</div></section>;
}
