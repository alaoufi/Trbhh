...
export default async function PaymentResultPage({ searchParams }: { searchParams: Promise<{ t?: string }> }) {
  ...
  const title = success ? 'تم الدفع بنجاح' : failed ? 'لم تكتمل عملية الدفع' : 'عملية الدفع قيد التحقق';
  const text = success ? 'أُضيف المبلغ إلى رصيدك تلقائياً بعد تأكيد البوابة.' : failed ? (topup.note || 'لم يُضف أي مبلغ إلى رصيدك. جرّب مجدداً أو استخدم التحويل البنكي.') : 'لم يصل تأكيد موثوق من البوابة بعد؛ لا يُضاف الرصيد حتى يكتمل التحقق.';
