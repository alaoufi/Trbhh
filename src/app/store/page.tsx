...
export default async function StoreAdminPage({ searchParams }: { searchParams: Promise<{ error?: string; sub?: string; added?: string; settings?: string; backup?: string; bulk?: string; bulklimit?: string; show?: string; adshow?: string; dupid?: string; price?: string; coupon?: string; renew?: string; plus?: string; staff?: string; other?: string; othername?: string; want?: string; exc?: string; mm?: string; nreq?: string; areq?: string; vreq?: string; cat?: string; banned?: string; newstore?: string }> }) {
  ...
  // الرصيد المتاح (الكلي − المحجوز) — يُعتمد لحجب طلب التوثيق (لا يُحجز إلا من المتاح)
  const merchAvail = store ? await (await import('@/lib/wallet')).getAvailableBalance(session.uid).catch(() => merchBalance) : 0;
        ...
        <div id="trbhh-show" className="card-3d scroll-mt-20 space-y-3 rounded-2xl p-4">
          ...
          {verifyPkgs.length > 0 && (
            <div className="space-y-2 rounded-xl border border-sky-300 bg-sky-50/50 p-3">
              ...
              <p className="text-[11px] text-muted-foreground">اختر باقة — <b>أول توثيق</b> بموافقة إدارة المتاجر: يُحجز مبلغ الرسوم من رصيدك <b>دون خصم</b> ويُخصم عند الموافقة أو يُعاد إن رُفض. و<b>التجديد لاحقاً فوري</b> بالخصم مباشرة بلا موافقة. وعند أي إلغاء تُعاد لرصيدك قيمة الأيام غير المستخدمة تلقائياً.</p>
              {vreq === 'ok' && <div className="rounded-lg border border-emerald-300 bg-emerald-50 p-2 text-xs font-bold text-emerald-800">✓ أُرسل طلب التوثيق وحُجز مبلغ الرسوم من رصيدك (دون خصم) — يُخصم عند الموافقة أو يُعاد إن رُفض.</div>}
              ...
              {vreq === 'balance' && <div className="rounded-lg border-2 border-amber-400 bg-amber-50 p-2 text-xs font-bold text-amber-900">💳 رصيدك المتاح لا يكفي — لم يُرسل الطلب. <Link href="/account/wallet#topup" className="text-primary underline">اشحن رصيدك</Link> ثم أعد المحاولة.</div>}
              ...
              {verifyActive ? (
                ...
                <div className="rounded-lg bg-amber-100 px-2 py-1.5 text-xs font-bold text-amber-800">⏳ طلبك (باقة {myVerify.fee} ر.س / {myVerify.days} يوم) بانتظار موافقة إدارة المتاجر — المبلغ <b>محجوز</b> من رصيدك (دون خصم)، يُخصم عند الموافقة أو يُعاد إن رُفض. رصيدك المتاح: {merchAvail} ر.س.</div>
              ) : (() => {
                // تجديد: آخر توثيق مدفوع انتهت مدّته طبيعياً (status 4) ⇒ فوري بالخصم بلا موافقة.
                // الرفض (2) والإلغاء الإداري (3) يعودان لمسار «أول مرة» (بموافقة + حجز).
                const isRenewal = myVerify?.status === 4;
                const cheapest = Math.min(...verifyPkgs.map((p) => p.fee));
                // الحجب على الرصيد المتاح في الحالتين: التجديد يخصم فوراً، وأول توثيق يحجز — كلاهما من المتاح.
                const canAffordAny = merchAvail >= cheapest;
                const firstAffordIdx = Math.max(0, verifyPkgs.findIndex((p) => p.fee <= merchAvail));
                return (
                <>
                  ...
                  {isRenewal ? (
                    <div className="rounded-lg border border-sky-300 bg-sky-50 p-2 text-xs font-bold text-sky-800">⌛ انتهت مدة توثيقك السابق — التجديد <b>فوري</b> بالخصم من رصيدك مباشرةً بلا انتظار موافقة (رصيدك المتاح: {merchAvail} ر.س).</div>
                  ) : (
                    <div className="rounded-lg border border-sky-300 bg-sky-50 p-2 text-xs font-bold text-sky-800">🔒 عند الإرسال يُحجز مبلغ الباقة من رصيدك <b>دون خصم</b> — يُخصم عند موافقة الإدارة أو يُعاد إن رُفض. لا يُرسَل الطلب إلا إن كفى رصيدك المتاح ({merchAvail} ر.س).</div>
                  )}
                  {!canAffordAny ? (
                    <div className="rounded-lg border-2 border-red-400 bg-red-50 p-2 text-xs font-bold text-red-700">💳 رصيدك المتاح ({merchAvail} ر.س) لا يكفي لأقل باقة توثيق ({cheapest} ر.س) — <Link href="/account/wallet#topup" className="underline">اشحن رصيدك</Link> {isRenewal ? 'ثم جدّد.' : 'ثم اطلب التوثيق.'}</div>
                  ) : (
                  <form action={requestVerifyPaidAction} className="space-y-2">
                    {/* اختيار الباقة — تُعطَّل الباقات التي لا يغطيها الرصيد المتاح */}
                    <div className={`grid gap-2 ${verifyPkgs.length === 3 ? 'grid-cols-3' : verifyPkgs.length === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                      {verifyPkgs.map((pkg, i) => {
                        const afford = pkg.fee <= merchAvail;
                        return (
