  ...
  return (
    <section ref={root} className="space-y-3 rounded-xl border-2 border-sky-200 bg-sky-50/50 p-3">
      ...
      <label className="block text-xs font-bold text-sky-900">
        تصميم البانر
        <select name="bannerTemplate" defaultValue="heritage" onChange={(event) => { const template = event.target.value as TopupCampaignPresentation['template']; setPresentation((current) => ({ ...current, template })); }} className="mt-1 w-full rounded-lg border bg-white p-2 text-sm">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="text-xs font-bold text-sky-900">
          عرض البانر
          <select name="bannerWidth" defaultValue="full" onChange={(event) => { const width = event.target.value as TopupCampaignPresentation['width']; setPresentation((current) => ({ ...current, width })); }} className="mt-1 w-full rounded-lg border bg-white p-2 text-sm">
            {TOPUP_BANNER_WIDTHS.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
          </select>
        </label>
        <label className="text-xs font-bold text-sky-900">
          ارتفاع البانر
          <select name="bannerHeight" defaultValue="medium" onChange={(event) => { const height = event.target.value as TopupCampaignPresentation['height']; setPresentation((current) => ({ ...current, height })); }} className="mt-1 w-full rounded-lg border bg-white p-2 text-sm">
