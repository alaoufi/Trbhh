...
export function CategoryPicker({options,labels,initialCategory=0,initialBranch=null,values={}}:{options:CategoryOption[];labels:Labels;initialCategory?:number;initialBranch?:number|null;values?:Record<string,string>}) {
  ...
  return <section className="space-y-4 rounded-2xl border bg-slate-50 p-4">
    ...
    {!!cat?.fields.length&&<fieldset key={id} className="grid gap-4 sm:grid-cols-2"><legend className="mb-3 font-bold">{labels.details}</legend>{cat.fields.map(f=><label key={f.id} className="space-y-1"><span className="block text-sm">{f.label}{f.required?' *':''}</span>{f.type==='select'?<select name={`category_field_${f.id}`} defaultValue={id===initialCategory?values[f.id]||'':''} required={f.required&&id!==initialCategory} className={cls}><option value="">اختر</option>{f.options.map(o=><option key={o}>{o}</option>)}</select>:<input name={`category_field_${f.id}`} type={f.type==='number'?'number':'text'} min={f.type==='number'?0:undefined} step={f.type==='number'?'any':undefined} maxLength={500} defaultValue={id===initialCategory?values[f.id]||'':''} required={f.required&&id!==initialCategory} className={cls}/>}</label>)}</fieldset>}
  </section>;
}
