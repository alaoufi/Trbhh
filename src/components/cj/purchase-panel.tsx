...
const failureMessage:Record<string,string>={checking:'جاري التحقق من المخزون والشحن...',out_of_stock:'غير متوفر حاليًا',quantity_exceeds_stock:'الكمية المطلوبة أكبر من المخزون المتاح',inventory_error:'تعذر التحقق من المخزون من CJ، حاول مرة أخرى',variant_changed:'تغيرت بيانات الخيار لدى المورد، حدّث الصفحة واختره مجددًا',no_shipping:'لا يوجد شحن متاح للسعودية لهذا الخيار',freight_error:'تعذر الحصول على سعر الشحن من CJ، حاول مرة أخرى',invalid_price:'تعذر التحقق من السعر الحالي، لا يمكن الإضافة'};
...
export function CjPurchasePanel({productId,productPid,productName,variants,accountId,isStaff}:{productId:number;productPid:string;productName:string;variants:Variant[];accountId:number;isStaff:boolean}){
  ...
  return <section aria-label="خيارات الشراء التجريبي" className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
    ...
    {quote&&<div className="space-y-3 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 text-sm">
      ...
      {ship?.deliveryDays&&<p>مدة الشحن حسب رد CJ: <b dir="auto">{ship.deliveryDays}</b></p>}
      {ship&&ship.additionalMinor>0&&<p className="flex justify-between"><span>رسوم إضافية حسب رد CJ</span><b>{money(ship.additionalMinor)}</b></p>}
    ...
    <p className="text-xs leading-6 text-slate-600">هذه تجربة للسلة فقط: لا خصم ولا حجز للمخزون ولا طلب لدى CJ. الشراء الحقيقي مقفل من إعداد المنصة.</p>
  </section>;
}
