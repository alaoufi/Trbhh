...
import { Phone, MessageCircle, ExternalLink, Pause, Play, LogIn, ChevronLeft } from 'lucide-react';
...
const DURATION = 6000; // ms per ad before auto-advancing / dismissing
...
export function ClassifiedSplash({ ads }: { ads: Classified[] }) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      setRemaining((r) => {
        ...
        if (next <= 0) {
          setIndex((i) => {
            if (i < total - 1) return i + 1; // advance to next ad
            clearInterval(iv);
            setShow(false);
            return i;
          });
  ...
  function next() {
    if (index < total - 1) { setIndex(index + 1); setRemaining(DURATION); }
    else setShow(false);
  }
  function enterSite() {
    setShow(false);
  ...
  const ad = ads[Math.min(index, total - 1)];
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center gap-4 bg-black/85 p-6 backdrop-blur-sm">
      {/* circular countdown */}
      <div className="relative h-14 w-14">
        <svg className="h-14 w-14 -rotate-90" viewBox="0 0 48 48">
          <circle cx="24" cy="24" r={R} fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="3.5" />
          <circle cx="24" cy="24" r={R} fill="none" stroke="#fff" strokeWidth="3.5" strokeLinecap="round" strokeDasharray={C} strokeDashoffset={C * (1 - progress)} style={{ transition: 'stroke-dashoffset 60ms linear' }} />
        </svg>
        <span className="absolute inset-0 flex items-center justify-center text-base font-bold text-white">{paused ? '⏸' : seconds}</span>
      </div>

      <div className="w-full max-w-xs">
        {ad.link ? <a href={ad.link} target="_blank" rel="noopener noreferrer" onClick={() => setShow(false)} className="block">{card}</a> : card}
        {(wa || ad.phone) && (
      ...
      {total > 1 && (
        <div className="flex gap-1.5">
          {ads.map((_, i) => <span key={i} className={`h-1.5 rounded-full transition-all ${i === index ? 'w-4 bg-white' : 'w-1.5 bg-white/40'}`} />)}
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        {total > 1 && (
          <button onClick={next} className="flex items-center gap-1.5 rounded-full bg-white/15 px-4 py-2 text-sm font-medium text-white hover:bg-white/25">
            التالي <ChevronLeft className="h-4 w-4" />
          </button>
        )}
        <button onClick={enterSite} className="flex items-center gap-1.5 rounded-full bg-white px-4 py-2 text-sm font-bold text-primary hover:bg-white/90">
          <LogIn className="h-4 w-4" /> الدخول للموقع
        </button>
      </div>
      <p className="text-xs text-white/60">
        {total > 1 ? `إعلان ${index + 1} من ${total}` : paused ? 'العرض متوقف' : 'ينتقل للموقع تلقائياً'}
      </p>
    </div>
  );
}
