...
export function CommerceCheckoutForm({ id, requestKey, memberName, maximum, regions, areas, submitLabel }: {
  ...
  return <form action={action} className="grid gap-3 sm:grid-cols-2">
    ...
    <button disabled={pending} className="rounded-lg bg-primary px-4 py-2 font-bold text-white disabled:opacity-50">{submitLabel}</button>
  </form>;
}
