...
export function HeaderSearch() {
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label="بحث" className="grid h-11 w-11 shrink-0 place-items-center rounded-full text-primary transition hover:bg-primary/10">
        <Search className="h-5 w-5" />
      </button>

      {open && (
        <div className="absolute inset-x-0 top-0 z-50 bg-accent/95 backdrop-blur">
          <form action="/search" className="container flex h-16 items-center gap-2">
            ...
            <div className="relative flex-1">
              <input ref={ref} name="q" placeholder="أبحث هنا"
              ...
              <div className="absolute left-2 top-1/2 flex -translate-y-1/2 items-center gap-2 text-primary">
                <Link href="/search" aria-label="بحث متقدم" onClick={() => setOpen(false)}><SlidersHorizontal className="h-5 w-5" /></Link>
