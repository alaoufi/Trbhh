...
export async function OpenStoreBanner() {
  ...
  return (
    <Link
      href="/open-store"
      className="relative flex items-center justify-between gap-3 overflow-hidden rounded-2xl p-4 text-white shadow-sm ring-1 ring-white/20 transition hover:-translate-y-0.5"
      style={{ backgroundImage: 'linear-gradient(110deg,#142b45,#234768)' }}
    >
      <span className="relative flex min-w-0 items-center gap-3">
        <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white/25 shadow-inner ring-1 ring-white/30"><Store className="h-6 w-6" /></span>
        <span className="min-w-0">
          <span className="flex flex-wrap items-center gap-2 text-base font-extrabold drop-shadow">
            ...
            {trialDays > 0 && (
              <span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-extrabold text-[#1d4ed8] shadow">تجربة مجانية {trialDays} أيام</span>
            )}
          </span>
