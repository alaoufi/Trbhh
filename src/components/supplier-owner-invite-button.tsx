...
export function SupplierOwnerInviteButton({supplierId,supplierName}:{supplierId:string;supplierName:string}){
 ...
 return <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-4" aria-label={`تفويض متجر ${supplierName}`}>
  <h3 className="font-bold text-emerald-950">تفويض صاحب المتجر — {supplierName}</h3>
  <p className="mt-1 text-sm leading-6 text-emerald-900">أنشئ رابطًا مؤقتًا صالحًا 24 ساعة وأرسله لصاحب المتجر. يوافق من حسابه في سلة دون إرسال أي إعدادات تقنية.</p>
  {!url&&<button type="button" className={`${primary} mt-3`} disabled={pending} onClick={issue}>{pending?'جارٍ إنشاء الرابط…':'إنشاء رابط تفويض لصاحب المتجر'}</button>}
  ...
  {url&&<div className="mt-3 space-y-3">
   ...
   <div className="flex flex-wrap gap-2"><button type="button" className={primary} onClick={async()=>{try{await navigator.clipboard.writeText(url);setCopied(true);}catch{setCopied(false);}}}>{copied?'تم نسخ الرابط':'نسخ الرابط'}</button><a href={url} target="_blank" rel="noreferrer" className={secondary}>فتح رابط التفويض</a></div>
