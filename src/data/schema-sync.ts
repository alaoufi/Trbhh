...
/** ترقية مالك منصّة العقار لمدير (قاعدتها منفصلة عن الإنتاج وتبدأ بلا مدير).
 *  يضمن أن صاحب الرقم المحدّد يملك صلاحية المدير الكامل (is_admin=1) في كل إقلاع
 *  — غير مشروط، حتى لو وُجد مدير آخر. عمليّة idempotent وآمنة. لا تعمل إلا في بيئة
 *  العقار (REALESTATE_ONLY=1) فلا تمسّ الإنتاج إطلاقاً. */
async function bootstrapFirstAdmin(): Promise<void> {
  try {
    // الرقم يُخزَّن بالصيغة المحلّية 05XXXXXXXX (وuserName=نفسه)؛ نطابق الصيغ الشائعة احتياطاً
    await prisma
      .$executeRawUnsafe(
        `UPDATE users SET is_admin = 1
         WHERE phoneNumber IN ('0548999968', '966548999968', '+966548999968', '00966548999968', '548999968')
            OR userName   IN ('0548999968', '966548999968')`,
      )
      .catch(() => {});
  } catch {
    /* تجاهل — الترقية لا تُعيق الإقلاع */
  }
}
