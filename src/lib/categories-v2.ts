...
import { validateCategoryValues, type CategoryOption } from '@/domain/category-fields';
...
export async function categoryOptions(): Promise<CategoryOption[]> {
  ...
  const [cats, subs, fields] = await Promise.all([
    ...
    prisma.category_field_defs.findMany({ where: {active:1}, orderBy:{ordered:'asc'} }),
  ]);
  return cats.map((cat) => ({ id:Number(cat.id),name:cat.name,active:cat.is_active==='yes',
    ...
    fields:fields.filter(f=>f.category_id===cat.id).map(f=>({id:Number(f.id),key:f.field_key,label:f.label,type:f.field_type,options:(f.options||'').split('|').filter(Boolean),required:f.required===1})),
  }));
}
...
export async function validateAdCategory(form: FormData, current?: {category_id:bigint;subcategory_id:number|null}) {
  ...
  const values=validateCategoryValues(cat.fields,input,!!retaining);
