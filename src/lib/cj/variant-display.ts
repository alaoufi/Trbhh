...
export function cjVariantDisplayOptions(variant:Pick<CjVariant,'variantKey'|'variantName'|'attributes'>):DisplayOption[]{
  ...
  for(const [key,value] of Object.entries(attrs)){
    const clean=safeText(value);if(!clean||/^(?:vid|pid|sku|id|price|weight)$/i.test(key))continue;
    options.push({label:labelFor(key),value:clean,source:'attribute'});
  }
  if(options.length)return options;
  const source=(variant.variantKey||variant.variantName||'').trim();
  if(!source)return [];
  const labeled=source.split(/\s*[,;|]\s*/).flatMap(part=>{
    const match=part.match(/^([^:=]+)\s*[:=]\s*(.+)$/);return match?[{label:labelFor(match[1]),value:match[2].trim(),source:'parsed' as const}]:[];
  });
  ...
  if(parts.length===2){
    ...
    if(sizeIndex>=0)return [{label:'اللون',value:parts[1-sizeIndex],source:'parsed'},{label:'المقاس',value:parts[sizeIndex],source:'parsed'}];
  }
