...
export async function readApprovedFiscalPolicy(db:Pick<CommerceDb,'$queryRaw'>,at:Date):Promise<ApprovedFiscalPolicy> {
 ...
 if(!issuer?.name?.trim()||!issuer.address?.trim()||!/^3\d{13}3$/.test(issuer.taxNumber)||!Number.isSafeInteger(vatBps)||vatBps<0||vatBps>10000||!row.policy_reference||fingerprint(fields)!==fingerprint({effectiveFrom:approved.effectiveFrom,issuer:approved.issuer,vatBps:approved.vatBps,policyReference:approved.policyReference,calculationPolicy:approved.calculationPolicy}))throw new Error('finance_issuance_not_approved');
