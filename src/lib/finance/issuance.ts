...
export async function issueProspectiveInvoice(db:CommerceDb,receiptId:bigint,authority:Authority,now=new Date()):Promise<string>{
 ...
 return db.$transaction(async tx=>{
  ...
  const items=await tx.$queryRaw<{product_id:bigint;title:string;quantity:number;unit_price_minor:number;total_minor:number}[]>`SELECT product_id,title,quantity,unit_price_minor,total_minor FROM commerce_order_items WHERE order_id=${order.id} ORDER BY product_id`;
