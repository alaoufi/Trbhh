...
import { calculateFiscalLines } from './calculations';
...
export async function auditFinance(tx:Tx,actor:bigint,action:string,entity:string,entityId:string,reason:string,payload:unknown={},now=new Date()) {
  await tx.$executeRaw`INSERT INTO finance_audit(created_at,actor_id,action,entity,entity_id,reason,payload) VALUES(${now},${actor},${action},${entity},${entityId},${reason},${JSON.stringify(canonical(payload))})`;
}
...
export async function requireOpenPeriod(tx:Tx,month:string) {
  ...
  const [period]=await tx.$queryRaw<{closed_at:Date|null}[]>`SELECT closed_at FROM finance_periods WHERE month=${month} FOR UPDATE`;
...
export async function saveBudget(db:CommerceDb,actor:bigint,input:{month:string;category:string;plannedMinor:number}) {
  const month=parseFinanceMonth(input.month),amount=money(input.plannedMinor);if(!categories.includes(input.category))throw new Error('finance_category_invalid');
  ...
  return db.$transaction(async tx=>{
    ...
    await audit(tx,actor,'budget_saved','budget',`${month}:${input.category}`,'تحديث الميزانية المخططة',{before:previous[0]?.planned_minor??null,after:amount});
  },options);
}
...
export async function releaseAccrual(db:CommerceDb,actor:bigint,accrualId:bigint,dueDate:string,reason:string,now=new Date()) {
  ...
  return db.$transaction(async tx=>{
    ...
    const [prior]=await tx.$queryRaw<{eligible_at:Date|null}[]>`SELECT eligible_at FROM finance_accrual_reviews WHERE accrual_id=${accrualId}`;
    ...
    await audit(tx,actor,'accrual_released','accrual',String(accrualId),reason,{dueAt:due},now);
  },options);
}
export async function accrualRemaining(tx:Tx,id:bigint,supplierId:bigint,now:Date,requireDue=true):Promise<number> {
  ...
  for(const note of notes)for(const line of financeJson<FiscalSnapshot>(note.snapshot).lines){
    if(line.key===String(row.product_id)&&line.supplierId===String(supplierId))adjustment+= (note.kind==='credit_note'?-1:1)*(line.supplierMinor??0);
  }
  return financeNumber(row.amount_minor)+adjustment-paid.reduce((sum,v)=>sum+(v.reversal_of?-1:1)*financeNumber(v.amount_minor),0);
}
...
export async function approveSettlement(db:CommerceDb,actor:bigint,settlementId:bigint,reference:string,now=new Date()) {
  ...
  return db.$transaction(async tx=>{
    await requireOpenPeriod(tx,financeMonth(now));
    const [row]=await tx.$queryRaw<{id:bigint;supplier_id:bigint;status:string;amount_minor:bigint;reference:string;reversal_of:bigint|null}[]>`SELECT * FROM finance_settlements WHERE id=${settlementId} FOR UPDATE`;
    ...
    await audit(tx,actor,'settlement_approved','settlement',String(settlementId),'إثبات تحويل منفذ خارج النظام',{reference,amountMinor:row.amount_minor},now);
  },options);
}
export async function reverseSettlement(db:CommerceDb,actor:bigint,settlementId:bigint,reason:string,now=new Date()) {
  ...
  return db.$transaction(async tx=>{
    ...
    await audit(tx,actor,'settlement_reversed','settlement',String(row.id),reason,{originalId:settlementId},now);return row.id;
  },options);
}
...
export async function captureInvoices(db:CommerceDb,actor:bigint) {
  await assertFinanceSchemaReady(db);const data=await readFinanceData(db);let created=0;
...
export async function issueInvoice(db:CommerceDb,actor:bigint,invoiceId:bigint,snapshot:FiscalSnapshot,gate:{enabled:boolean;approvedPolicyReference:string},now=new Date()) {
  ...
  return db.$transaction(async tx=>{
    ...
    await audit(tx,actor,'invoice_issued','invoice',String(invoiceId),'اعتماد اللقطة وفق سياسة جهة الإصدار',{number,policyReference:snapshot.policyReference},now);return number;
  },options);
}
...
export async function closeMonth(db:CommerceDb,actor:bigint,month:string,checks:string[],reason:string,now=new Date()) {
  ...
  return db.$transaction(async tx=>{
    ...
    await audit(tx,actor,'month_closed','period',month,reason,{checks,metrics:report.metrics},now);
  },options);
}
export async function recordFinanceExport(db:CommerceDb,actor:bigint,month:string,format:string,section:string) {
  await assertFinanceSchemaReady(db);return db.$transaction(tx=>audit(tx,actor,'report_exported','report',parseFinanceMonth(month),'تصدير تقرير مالي',{format,section}),options);
}
