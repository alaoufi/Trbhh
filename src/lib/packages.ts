...
export type Package = {
  ...
  tier: Tier;           // ذهبي / فضي for featured packages
...
export const FREE_FALLBACK: Package = {
  ...
  featuredSlots: 0, featuredDays: 0, tier: '', isDefault: true, sort: 0, active: true,
};

type Row = {
  id: number; name: string; price: unknown; ads_per_day: number; gap_hours: number;
  featured_slots: number; featured_days: number; tier: string; is_default: number; sort: number; active: number;
};
...
export async function createPackage(p: Omit<Package, 'id'>) {
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO packages (name, price, ads_per_day, gap_hours, featured_slots, featured_days, tier, is_default, sort, active)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    p.name, p.price, p.adsPerDay, p.gapHours, p.featuredSlots, p.featuredDays, p.tier, p.isDefault ? 1 : 0, p.sort, p.active ? 1 : 0,
  );
}
...
export async function updatePackage(id: number, p: Omit<Package, 'id'>) {
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE packages SET name = ?, price = ?, ads_per_day = ?, gap_hours = ?, featured_slots = ?, featured_days = ?, tier = ?, is_default = ?, sort = ?, active = ? WHERE id = ?`,
    p.name, p.price, p.adsPerDay, p.gapHours, p.featuredSlots, p.featuredDays, p.tier, p.isDefault ? 1 : 0, p.sort, p.active ? 1 : 0, id,
  );
}
