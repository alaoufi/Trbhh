...
function eligible(row:Row):boolean{return row.connection_provider==='salla'&&row.profile_provider==='salla'&&row.connection_status==='connected'&&row.connection_scope_version===SALLA_OAUTH_SCOPE_VERSION&&Number(row.connection_has_tokens)===1&&row.supplier_active===1&&row.maintenance===0;}
...
export async function loadCatalogDetail(db:Reader,key:string):Promise<CatalogDetail>{
 const id=keyId(key),[row]=await db.$queryRaw<Row[]>(Prisma.sql`SELECT ${fields},p.images,p.description,p.brand,p.categories,p.options,p.variants ${joins} WHERE p.id=${id} AND c.provider='salla' AND ip.provider='salla' LIMIT 1`);
