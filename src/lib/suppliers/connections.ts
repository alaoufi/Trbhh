...
export async function beginOAuth(db:CommerceDb,supplierId:bigint,adminId:bigint,config:SupplierConfig) {
...
export async function consumeOAuthState(db:CommerceDb,state:string,browser:string,adminId:bigint):Promise<bigint> {
  if(!/^[a-f0-9]{64}$/.test(state)||!/^[a-f0-9]{64}$/.test(browser)||adminId<=0n)throw new Error('supplier_oauth_state');
  return db.$transaction(async tx=>{
    const [row]=await tx.$queryRaw<{supplier_id:bigint}[]>`SELECT supplier_id FROM supplier_oauth_states WHERE state_hash=${digest(state)} AND browser_hash=${digest(browser)} AND admin_id=${adminId} AND consumed_at IS NULL AND expires_at>UTC_TIMESTAMP(3) FOR UPDATE`;
...
export async function completeOAuth(db:CommerceDb,input:{state:string;browser:string;adminId:bigint;code:string},config:SupplierConfig,fetcher:typeof fetch=fetch) {
  assertOAuthConfig(config);
  if(!input.code||input.code.length>8192)throw new Error('supplier_oauth_code');
  const supplierId=await consumeOAuthState(db,input.state,input.browser,input.adminId);
