...
const shipping={name:'Fixture',phone:'+966500000000',addressLine:'Fixture street',city:'Riyadh',postalCode:'12345',country:'SA' as const};
let db:PrismaClient,admin:PrismaClient,created=false;
const tables=['supplier_webhook_events','supplier_shipments','supplier_coordinator_notifications','supplier_order_sync_attempts','supplier_orders','supplier_reservation_allocations','supplier_stock_reservations','supplier_price_tiers','supplier_price_history','supplier_products','supplier_oauth_states','supplier_connections','supplier_integration_profiles','commerce_supplier_accruals','commerce_receipts','commerce_order_suppliers','commerce_product_suppliers','commerce_notifications','commerce_audit_events','commerce_payment_attempts','commerce_order_items','commerce_orders','commerce_products','commerce_suppliers'];
describe.skipIf(process.env.SUPPLIER_DB_TESTS!=='1')('isolated supplier MySQL proof',()=>{
 beforeAll(async()=>{const url=new URL(process.env.SUPPLIER_TEST_DATABASE_URL||'');if(url.protocol!=='mysql:'||url.hostname!=='127.0.0.1'||url.port!=='33309'||url.pathname!=='/trbhh_supplier_test'||url.search||url.hash)throw new Error('Refusing non-isolated supplier DB');db=new PrismaClient({datasourceUrl:url.href,log:[]});url.pathname='/mysql';admin=new PrismaClient({datasourceUrl:url.href,log:[]});await admin.$executeRawUnsafe('CREATE DATABASE trbhh_supplier_test');created=true;for(const ddl of [...COMMERCE_DDL,...SUPPLIER_DDL])await db.$executeRawUnsafe(ddl);await assertSupplierSchemaReady(db);});
 ...
 beforeEach(async()=>{for(const table of tables)await db.$executeRawUnsafe(`DELETE FROM ${table}`);await db.$executeRaw`INSERT INTO commerce_suppliers(id,name,active) VALUES(1,'Fixture supplier',1)`;await db.$executeRaw`INSERT INTO supplier_integration_profiles(supplier_id,provider,sync_enabled,auto_orders_enabled) VALUES(1,'salla',1,1)`;await db.$executeRaw`INSERT INTO supplier_connections(id,supplier_id,provider,external_store_id,status,encrypted_tokens,expires_at) VALUES(1,1,'salla','999','connected',${sealTokens({accessToken:'fixture-access',refreshToken:'fixture-refresh'},'salla:1:999',config.encryptionKey)},${new Date(Date.now()+86400000)})`;});
 ...
 beforeEach(async()=>{await db.$executeRaw`DELETE FROM notfications`;await db.$executeRaw`DELETE FROM site_settings`;});
