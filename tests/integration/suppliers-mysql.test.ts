...
describe.skipIf(process.env.SUPPLIER_DB_TESTS!=='1')('isolated supplier MySQL proof',()=>{
 beforeAll(async()=>{const url=new URL(process.env.SUPPLIER_TEST_DATABASE_URL||'');if(url.protocol!=='mysql:'||url.hostname!=='127.0.0.1'||url.port!=='33309'||url.pathname!=='/trbhh_supplier_test'||url.search||url.hash)throw new Error('Refusing non-isolated supplier DB');db=new PrismaClient({datasourceUrl:url.href,log:[]});url.pathname='/mysql';admin=new PrismaClient({datasourceUrl:url.href,log:[]});await admin.$executeRawUnsafe('CREATE DATABASE trbhh_supplier_test');created=true;await db.$executeRawUnsafe("CREATE TABLE users(id BIGINT UNSIGNED NOT NULL PRIMARY KEY,name VARCHAR(255) NULL,ban ENUM('checked','no') NULL DEFAULT 'no',ban_until DATETIME NULL,archived_at DATETIME NULL,merged_into BIGINT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin");for(const ddl of [...ACCESS_CONTROL_DDL,...COMMERCE_DDL,...CJ_DDL,...SUPPLIER_DDL,...ONBOARDING_DDL,...FINANCE_DDL])await db.$executeRawUnsafe(ddl);await assertSupplierSchemaReady(db);});
