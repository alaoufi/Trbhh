...
const state=vi.hoisted(()=>({allowed:true,edit:false,suspend:false,remove:false,cache:new Map<string,string>(),savedRows:new Map<string,string>(),read:vi.fn(),write:vi.fn(),translate:vi.fn(),sourceName:'Cotton Summer Shirt',nameAr:'',category:'Summer Clothing',categoryPath:'Clothing > Summer Clothing',variantName:'Blue Large'}));
...
beforeEach(()=>{vi.clearAllMocks();state.allowed=true;state.edit=false;state.suspend=false;state.remove=false;state.cache=new Map();state.savedRows.clear();state.sourceName='Cotton Summer Shirt';state.nameAr='';state.category='Summer Clothing';state.categoryPath='Clothing > Summer Clothing';state.variantName='Blue Large';});
