...
const state=vi.hoisted(()=>({user:vi.fn(),config:vi.fn(),gateway:vi.fn(),attempt:vi.fn(),address:vi.fn(),products:vi.fn(),createOrder:vi.fn(),redirect:vi.fn((url:string):never=>{throw new Error(`redirect:${url}`);})}));
