...
const mock=vi.hoisted(()=>({config:vi.fn(),product:vi.fn(),similar:vi.fn(),policy:vi.fn()}));
...
vi.mock('@/lib/commerce/public-product',()=>({readPublicCommerceProduct:mock.product,readSimilarPublicCommerceProducts:mock.similar}));
...
beforeEach(()=>{vi.clearAllMocks();mock.config.mockResolvedValue({enabled:true,purchasingEnabled:false,paymentsEnabled:false,shippingFeeMinor:125,text:{shippingTerms:'شروط تجريبية',unavailable:'الشراء غير متاح'}});mock.product.mockResolvedValue(product);mock.similar.mockResolvedValue([]);mock.policy.mockResolvedValue(policy(false));});
describe('commerce product page VAT and purchase gate',()=>{
 it.each(['inclusive','exclusive'] as const)('does not claim VAT is included or add it to display when central VAT is OFF (%s)',async basis=>{mock.policy.mockResolvedValue(policy(false,basis));const saved=JSON.stringify(policy(false,basis)),html=await render();expect(html).toContain('10.25');expect(html).not.toContain('شامل الضريبة');expect(html).not.toContain('قبل الضريبة');expect(html).toContain('رسوم التوصيل المعروضة حاليًا:');expect(html).toContain('الشراء والدفع غير متاحين حاليًا');expect(html).toContain('disabled=""');expect(JSON.stringify(policy(false,basis))).toBe(saved);});
