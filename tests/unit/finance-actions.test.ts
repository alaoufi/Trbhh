...
const mocks=vi.hoisted(()=>({requireAccess:vi.fn(),recordExpense:vi.fn(),saveBudget:vi.fn(),approveSettlement:vi.fn(),closeMonth:vi.fn(),revalidatePath:vi.fn()}));
...
beforeEach(()=>{vi.clearAllMocks();mocks.requireAccess.mockResolvedValue({uid:9});});
