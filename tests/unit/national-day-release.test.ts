...
describe('national day preservation release',()=>{
  it('runs real filesystem chain and historical raw-proof review in CI',()=>{
    const result=spawnSync(process.execPath,['--test','scripts/release/national-media-reference.test.cjs'],{encoding:'utf8',timeout:30000});
    expect(result.error).toBeUndefined();expect(result.status,result.stdout+'\n'+result.stderr).toBe(0);
  },35000);
  it('selects national_day only on exact feature branch and reviewed checkpoint',()=>{
    ...
    for(const [ref,reuse,profile] of [
      ...
      ['codex/supplier-visual-selection-20260921-other','35626587686','standard'],
      ['claude/hostinger-vps-project-amw8vb','35626587686','standard'],
  ...
  it('rejects an unreviewed checkpoint before any production access',()=>{
    ...
    for(const phase of ['before','after'])for(const reuse of ['35626587686','35607654850','123',"''"]){
      expect(shell(`set -- ${phase} 999 ${candidate} ${reuse} national_day\n${prefix}`).status===0).toBe(reuse==='35626587686');
    }
    expect(shell(`set -- before 35626587686 ${candidate} 35626587686 national_day\n${prefix}`).status).not.toBe(0);
  });
  it('rejects production baseline drift or wrong parent before archives',()=>{
    ...
    for(const [current,reuse,allowed] of [[baseline,'35626587686',true],[candidate,'35626587686',false],[baseline,'35607654850',false],[baseline,'',false]] as const){
      const result=shell(gate!,{release_profile:'national_day',current_commit:current,reuse_media_id:reuse});expect(result.status===0,result.stderr).toBe(allowed);
    }
  });
  ...
  it('keeps current after proof strict and completes it before recording success',()=>{
    ...
    for(const proof of ['national-media-reference.cjs" verify','verify "$backup/supplier-before.json" "$backup/supplier-after.json"','verify "$backup/before.json" "$backup/after.json"','verify-schema <','verify-runtime.cjs" "$backup/container-before.json" "$backup/container-after.json"','verify "$backup/$media-before.json" "$backup/$media-after.json"']){
      expect(script.indexOf(proof)).toBeGreaterThan(-1);expect(script.indexOf(proof)).toBeLessThan(finish);
    }
    expect(script.indexOf('national-media-reference.cjs" prepare')).toBeLessThan(script.indexOf('docker image save'));
    expect(script).toContain('"$release_profile" == national_day ]]; then media_capacity=verified-parent');
    expect(script).toContain('sha256sum NATIONAL_MEDIA_REFERENCE.json REUSED_MEDIA_SOURCE *-before.json *.path');
  });
  it('enforces runtime credentials and disabled live orders for national_day',()=>{
    ...
    function verify(changes:Record<string,string>={}){
      ...
      return ()=>runInNewContext(runtime,{require:(name:string)=>name==='node:fs'?{readFileSync:(file:string)=>JSON.stringify(fixtures[file])}:require(name),process:{argv:['node','verify-runtime','before','after','national_day'],exit:()=>{throw Error('denied');}},console:{error:()=>{},info:()=>{}}},{timeout:1000});
    }
