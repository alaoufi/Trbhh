...
const mocks=vi.hoisted(()=>({requireAction:vi.fn(),schema:vi.fn(),load:vi.fn(),detail:vi.fn(),review:vi.fn(),approve:vi.fn(),revalidate:vi.fn()}));
...
vi.mock('@/lib/suppliers/catalog-admin',()=>({loadCatalog:mocks.load,loadCatalogDetail:mocks.detail,reviewCatalogSelection:mocks.review,approveCatalogSelection:mocks.approve}));
vi.mock('next/cache',()=>({revalidatePath:mocks.revalidate}));
import {searchProducts,productDetails,reviewProducts,approveProducts} from '@/app/admin/suppliers/catalog/actions';
...
describe('supplier catalog action boundary',()=>{
 it.each([
  ...
  for(const fn of [mocks.load,mocks.detail,mocks.review,mocks.approve])expect(fn).not.toHaveBeenCalled();
 });
 it('requires edit permission even when a viewer can load the catalog',async()=>{
  ...
  await expect(approveProducts({token:'x',confirmed:true,products:[]})).rejects.toThrow('forbidden');expect(mocks.schema).not.toHaveBeenCalled();
 });
