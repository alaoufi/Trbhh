...
const mocks=vi.hoisted(()=>({requireAccess:vi.fn(),schema:vi.fn(),load:vi.fn(),detail:vi.fn(),review:vi.fn(),approve:vi.fn(),updateSale:vi.fn(),hide:vi.fn(),remove:vi.fn(),revalidate:vi.fn()}));
vi.mock('@/lib/access-control/guards',()=>({requireAccess:mocks.requireAccess}));
...
beforeEach(()=>{vi.clearAllMocks();mocks.requireAccess.mockResolvedValue({uid:7});mocks.schema.mockResolvedValue(undefined);});
describe('supplier catalog action boundary',()=>{
 it.each([
  ...
  mocks.requireAccess.mockRejectedValueOnce(Error('forbidden'));
 ...
 it('requires edit permission even when a viewer can load the catalog',async()=>{
  mocks.requireAccess.mockImplementation(async(_service,action)=>{if(action!=='view')throw Error('forbidden');return {uid:7};});
