...
describe('supplier OAuth status',()=>{
 it('shows the five operator states from durable profile and connection data',()=>{
  ...
  expect(resolveSupplierOAuthStatus({inviteExpiresAt:new Date('2026-09-22T13:00:00Z'),connection:null},now)).toMatchObject({key:'pending',label:'بانتظار تفويض صاحب المتجر'});
