...
function fixture(existing=false,email='test@example.com'){
 const supplier={id:1n,name:'متجر قديم',contact_name:'مفوض',phone:'+966501234567',email,address:'عنوان',registration_number:'1010123456',tax_number:'',settlement_terms:'شهري',notes:'احتفظ',active:0,updated_at:new Date()};
...
describe('registration import integrity',()=>{
 ...
 it('preserves a required profile value omitted from an existing supplier update but rejects it for a new supplier',async()=>{const partial:OnboardingValues={...values};delete partial.email;const existing=fixture(true),preview=await inspectOnboarding(existing.db,partial,secret);expect(preview.merged.email).toBe('test@example.com');await expect(saveOnboarding(existing.db,{values:partial,fingerprint:preview.fingerprint,filename:'store.xlsx',adminId:3n,secret,canCreate:false,canEdit:true})).resolves.toMatchObject({supplierId:'1'});const fresh=fixture(),freshPreview=await inspectOnboarding(fresh.db,partial,secret);await expect(saveOnboarding(fresh.db,{values:partial,fingerprint:freshPreview.fingerprint,filename:'store.xlsx',adminId:3n,secret,canCreate:true,canEdit:false})).rejects.toThrow('onboarding_validation');});
 it('allows a connected existing store with a legacy blank email and does not report another blank supplier as an email conflict',async()=>{const partial:OnboardingValues={...values};delete partial.email;const f=fixture(true,'');f.setEmail([{id:2n}]);f.setConnections([{id:3n,status:'connected',version:1}]);const preview=await inspectOnboarding(f.db,partial,secret);expect(preview.connected).toBe(true);expect(preview.warnings).toEqual([]);await expect(saveOnboarding(f.db,{values:partial,fingerprint:preview.fingerprint,filename:'store.xlsx',adminId:3n,secret,canCreate:false,canEdit:true})).resolves.toMatchObject({supplierId:'1'});});
