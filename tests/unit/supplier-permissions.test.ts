...
describe('independent supplier permissions', () => {
  it('registers view/add/edit without granting access to ordinary staff presets', () => {
    expect(SERVICES.find(s => s.key === 'suppliers')?.actions).toEqual(['view', 'add', 'edit']);
    expect(ALL_KEYS).toEqual(expect.arrayContaining(['suppliers:view', 'suppliers:add', 'suppliers:edit']));
