...
import {authorizationUrl,exchangeCode,refreshGrant,merchantIdentity,merchantStoreIdentity,assertMerchantStoreMatches,assertRequiredScopes,SALLA_REQUIRED_SCOPES} from '@/lib/suppliers/salla-oauth';
...
describe('Salla custom OAuth transport',()=>{
 it('requests every scope required by catalog, customer and order execution without a secret',()=>{const url=new URL(authorizationUrl(config,'a'.repeat(64)));expect(url.origin).toBe('https://accounts.salla.sa');expect(url.searchParams.get('redirect_uri')).toBe('https://trbhh.sa/api/integrations/salla/callback');expect(new Set(url.searchParams.get('scope')?.split(' '))).toEqual(new Set(SALLA_REQUIRED_SCOPES));expect(url.href).not.toContain('test-secret');});
 it('requires a fresh grant that explicitly includes customer write access',()=>{
  expect(()=>assertRequiredScopes('offline_access products.read customers.read orders.read orders.read_write metadata.read')).toThrow('salla_reauthorization_required');
  expect(assertRequiredScopes(SALLA_REQUIRED_SCOPES.join(' '))).toEqual([...SALLA_REQUIRED_SCOPES]);
 });
